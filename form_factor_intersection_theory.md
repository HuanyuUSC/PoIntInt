...
$V_{\cap}(t)=\frac{1}{(2\pi)^3}\int e^{-\|k\|^2/\kappa^2} F_1(k) \overline{F_2(k)} e^{i k\cdot t} dk
+\iint \chi_{\Omega_1}(x) \chi_{\Omega_2}(y)  g_\kappa(x-y-t) dx dy,$
...
$J_\triangle(k;a,e_1,e_2)=\int_{u\ge0} \int_{v\ge0,\ u+v\le1} e^{i k\cdot(a+u e_1+v e_2)} du dv
...
$V_{\cap}(t)=\sum_{n,\ell,m} c_{n\ell m}^{(1)}  \overline{c_{n\ell m}^{(2)}}  e^{i \phi_{n\ell m}(t)}.$
